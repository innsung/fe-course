...
/********************************
    ...
    use tables;		--데이터베이스의 모든 테이블 조회(show)
********************************/
...
-- 사원테이블의 사원명, 폰번호, 급여, 보너스(급여의 10%), 압시일 조회
-- 산술연삭식이 가능한 컬럼은 Number 타입 가능
desc employee;
select emp_name, phone, salary, salary*0.1 as bouns, hire_date
	from employee;
...
-- sys 부서에 속한 모든 사원을 조회, 단 출력되는 EMP_ID컬럼은 '사원번호' 별칭으로 조회
select emp_id as '사원번호', emp_name, hire_date, salary
	from employee 
...
-- 재직중인 사원들을 조회 : 퇴사일자 컬럼 - retire_date
select *
	from employee
...
***********************************************/  
-- 사원테이블에서 부서아이디를 조회
-- 사원태이블의 부서아이디는 부서테이블의 부서아이디를 참조하므로 다중 조회
...
select  emp_id,						-- (3)
        ...
        salary as '급여'
	from employee					-- (1)
    where retire_date is not null	-- (2) 별칭 사용 ❌
    order by salary desc;			-- (4) 별칭 사용 가능
...
***********************************************/ 
-- 급여가 5000이상인 사원들을 조회, 급여를 오름차순으로 정렬
select * 
	from employee
    where salary >=5000
    order by salary;
...
-- ~이고 : AND - 두 개의 조건이 모두 만족해야하만 조회
select *
	from employee
    where hire_date > '2015-01-01' and salary >= 6000;
    
-- 특정범위 : 2015-01-01 ~ 2017-12-31 사이에 입사한 모든 사원 조회
select *
	from employee
...
-- 이메일 두번째 자리에 'a'가 들어오는 모든 사원들을 조회
select *
	from employee
    where email like '_a%';
...
-- 3자리 수를 랜덤으로 발생시켜, 2로 나누어 홀수, 짝수로 구분
...
-- (6) format(문자열, 소수점자리) : 문저열 포맷 - 3자리 콤마 구분
select format(123456, 2) as format1,
...
select  emp_id,
        ...
        concat(format(salary*0.2, 0), '원') as bouns
	from employee
...
-- 영업부서 사원의 사원명, 폰번호, 부서명, 휴가사용 이유, 소속본부 조회 조회
...
-- 2014년 부터 2016년 까지 입사한 사원들 중에서 퇴사하지 않은 사원들의
-- 사원아이디, 부서명, 입사일, 소속본부를 조회
-- 소속본부 기준으로 오름차순 정렬
select  e.emp_id,
		d.dept_name,
        e.hire_date,
...
select dept_name, unit_id, start_date, dept_name dept_id 
	from department
    order by dept_id;
...
select  d.dept_id,
		d.dept_name,
		ifnull(u.unit_name,'준비중') as uint
...
select  e.emp_name,
	...
	from (select emp_name, hire_date, salary, dept_id
			from employee 
			where retire_date is null) e right outer join department d on e.dept_id = d.dept_id
					left outer join unit u on d.unit_id = u.unit_id;
...
/***********************************************
    ...
    형식> SELECT [컬럼리스트 -> (스칼라 서브쿼리)] 오라클 사용X
		 FROM [테이블명 -> (인라인뷰)]
		 WHERE [조건절 -> (서브쿼리)]
***********************************************/  	
-- [서브쿼리]
...
select  vacation_id,
        ...
        (select emp_name from employee 
				where emp_name='홍길동') as emp_name,		-- 권장X
        begin_date,
        end_date,
        reason,
        duration
...
select * from vacation
	where emp_id in (select emp_id from employee where dept_id  
						in (select dept_id from department where unit_id = 
								(select unit_id from unit where unit_name = '제3본부'))); 

-- 휴가를 한번이라도 사용한 모든 사원 조히
select * 
	from employee e
...
-- 2) 부서테이블과 조인하여 모든 부서의 총 급여, 평균 급여 출력
select  d.dept_id,
		d.dept_name,
        d.unit_id,
        d.start_date,
...
-- 3) 사원테이블을 조인하여, 사원명, 급여, 부서아이디, 부서명, 총급여, 평균급여를 조회
...
as
 ...
 select emp_name,
        ...
        (select floor(avg(salary)) from employee where dept_id='SYS') as '평균급여'
	from employee 
    where dept_id = 'SYS';
...
/***********************************************
	제약사항(Constraints) : 데이터 무결성 원칙을 적용하기 위한 규치
...
/*************************************************************************************
    ...
    형식> ALTER TABLE [테이블명]
        ...
        MODIFY CONSTRAINT[제약사항명] 제역사항(컬럼)
        DROP [제약사항명]
...
alter table emp_const 
	add phone char(13);
select * from emp_const;
-- phone 컬럼에 null 값을 '010-1111-1234'
...
desc emp_const;		-- not tnull, default, check(?) 제약 확인
select * from information_schema.table_constraints
...
-- salary 값 입력시 '3000' 이상인 값만 저장되도록 check
alter table emp_const3
...
alter table emp_const2
	add constraint fk_department2_dept_id foreign key(did)
...
-- department2 테이블의 SYS부서 삭제한다면... emp_const2 에서 참조하므로, 삭제불가!!
-- (1) emp_const2 테이블의 SYS 참조 행 삭제
-- (2) department 테이블의 SYS 데이터 삭제 가능
...
alter table emp_const2
	drop constraint fk_department2_dept_id;
...
-- department2 의 'ACC' 부서를 'ABC' 부서로 수정
update department2 set dept_id = 'ABC' where dept_id = 'ACC';

select * from department2;

-- departemnt2의 'ABC' 부서를 삭제
...
-- 정보시스템 사원들 연봉이 높은 순으로, 사번, 사원명, 급여 조회
...
-- 휴가를 많이 사용한 사원순으로 조회, 행번호, 사원명, 부서아이디, 휴가일수
select  row_number() over(order by sum(v.duration) desc) as rno,
		e.emp_id,
		e.dept_id,
        sum(v.duration)
	from employee e, vacation v
...
-- 입사일이 빠른 순으로 정렬
select rank() over(order by hire_date asc) as '입사일순서',
		emp_id,
		emp_name,
        salary,
        hire_date
...
/*************************************************************************************
	트리거 생성 : 테이블의 pk정의 :: '문자'+'0001'(auto_increment)
*************************************************************************************/

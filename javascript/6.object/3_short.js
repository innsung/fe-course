/**
 * 변슈를 객체의 값으로 정의하는 경우, property(key) 생략가능
 */
...
//사원의 이름, 나이, 주소, 소속부서 정보를 입력받아 사원객체를 생성하고 반환하는 함수
//createEmployee, arrow Function 사용

const createEmployee = (name, age, address, department) => {    //retrun 변수와 일치
    return{name, age, address, department}                      //retrun 변수와 일치
}
...
for(let i=0; i<empList.length; i++){
    let object = null;  //객체
    let name, age, address, department = undefined;   //값
    // let age = undefined;    //값
    // let address = undefined;    //값
    // let department = undefined; //값
    for(let j=0; j<empList[i].length; j++){

/**
 * 제어문 : 반복 실행
 * - for : 반복 횟수를 알고 있는 경우(배열)
 * - while : 종료 시점을 알고 있는 경우
 * 
 * for(1️⃣초기값 ; 2️⃣조건절(boolean) ; 4️⃣증가값) {
 *      3️⃣실행문;
 * }
 * - for문이 실행되는 순서 : 1 > 2 > 3 > 4 > [2 > 3 > 4] 반복 실행, 조건이 false 될 때 까지
 * - 초기값 선언시 키워드는 let 으로 정의
 */

//1부터 10까지 출력
//1️⃣초기값 - for 블록에서 사용되는 변수
//2️⃣조건절(boolean) - 10보다 작거나 같을 때 까지 계속 반복, 결과는 true/false
